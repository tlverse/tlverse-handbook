...
To put it in contex, Abacavir and Tenofovir are commonly prescribed as part of the 
...
assigning treatment on a population level. But how do we know which interention works for which 
...
as possible $V$ covariates. Defining $V$ allows us to consider possibly suboptimal rules
that are easier to estimate, and thereby allows for statistical inference for the couterfactual 
mean outcome under the suboptimal rule.
...
Many methods for learning the optimal rule from data have been developed \cite{murphy2003,robins2004,moodie2013,robins2014}. In this chapter, we focus on the methods 
...
Finally, we demonstrate how to evaluate the mean outcome under the optimal individualized treatment 
using `tmle3mopptx`. To start, let's load the packages we'll use and set a seed:
...
the treatment is a categorical variable; later in the chapter we will consider another 
data generating distribution where $A$ is binary. In this example, our data generating
...
mean under the true optimal rule is $\psi=0.625$, which is the quantity we aim 
to estimate.
...
the pseudo-blip. In addition, we need to specify whether we want to maximize the 
mean outcome under the rule (`maximize=TRUE`), and whether we want to estimate the 
...
optimal individualized rule will be based on the smaller subset. We explore this 
important feature of `tmle3mopttx` in the later sections. 
...
individualized treatment when $A$ is binary. While the procedure is analogous to 
the previously described categorical treatment, we now need to pay attention to
...
the blip function. Note that now we use a different metalearner for fitting the 
propensity score and the blip! We make the above explicit with respect to standard 
...
estimating the value of the ITR. First one considers a setting where the user 
might be interested in a grid of possible suboptimal rules, corresponding to 
potentially limited knowledge of potential effect modifers. The second extension
concerns implementation of realistic optimal individual interventions where
certain regimes might be prefered, but due to practial or global positivity 
...
for the main interention in WASH Benefits data.
...
We specify the Super Learner library as in the categorical simulation:
...
living conditions (floor), and possession of mateial items including the refrigerator.
...
4. Using the same simulation, perform a variable importance analysis using 
...
2. Consider simpler rules for the WASH benefits data example. What kind of 
