...
```{r, fig.cap="How a counterfactual outcome changes as the natural treatment distribution is shifted by a simple stochastic intervention", results = "asis", echo=FALSE}
...
`r seq(-1, 1, by = 1)` on the scale of the treatment $A$ (note that the
...
```{r fit_tmle_wrapper_vimshift, eval=FALSE}
tmle_fit <- tmle3(tmle_spec, data, node_list, learner_list)
tmle_fit
```
...
```{r shift-review-ex1-sol, echo=FALSE}
```
