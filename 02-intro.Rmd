...
data-generating distribution that produced the observed data.
...
$\mathcal{R}$. It is defined by the query mentioned at the end of Step 2. Target
...
appendix of this chapter for more detail).
...
example of a binary exposure $A$, a binary outcome $Y$, and one categorical
confounding variable $W$. The DAG for these relations is depicted below.
...
distribution in the population under two interventions: 1) $A$ is set to 1 for
all individuals, and 2) $A$ is set to 0 for all individuals. These interventions
...
target parameter as a function of the observed data distribution.
