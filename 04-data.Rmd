...
of all other variable measured at baseline. In this data, our outcome is 
occasionally missing, but we do not need to create a variable indicating this 
missingness (such as $\Delta$) for analyses in the `tlverse`, since it is 
automatically detected when `NA` are present in the outcome. We can denote our 
observed data structure as $n$ i.i.d. copies of $O_i = (W_i, A_i, \Delta_i, 
\Delta Y_i)$, for $i = 1, \ldots, n$, where $\Delta$ denotes the indicator that 
