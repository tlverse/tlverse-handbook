...
output is an individualized treatment for each patient [@bembom2007realistic, @robins1986, @moodie2013].
...
data examples and software demonstrations. For more information on the technical asspects
of the algorithm with further practical advice with overview, the interested reader is invited to additionally consult @vanderLaanLuedtke15, @luedtke2016super, @montoya2021optimal and @montoya2021performance. 
...
$V \rightarrow d(V) \in \{a_1, \cdots, a_{n_A} \} \times
...
a counterfactual $Y_{d(V)}$ defined by a modified system in which the equation
for $A$ is replaced by the rule $d(V)$, dependent on covariates $V$. For a given 
...
outcomes for a patient assigned treatment, or given control, are similarly written as 
...
who benefit the most (or the least) from treatment. Additionally, blip-based apprach
allows for straight forward extension to the categorical treatment, interpretable rules, 
and OIT under resource constrains, where only a percent of the population can receieve treatment [@luedtke2016resource].
...
relies on doing a very good job on estimating $\bar{Q}_{Y,0}(A,W)$. 
...
treatment is benefitial (according to the estimated blip). In order to impose a 
resource constraint, we have to specify the percent of individuals that will
benefit the most from getting treatment. For example, if `resource=1`, all
individuals with blip higher than zero will get treatment; if `resource=0`,
none will get treatment. 
...
Note that since the corresponding blip will be vector valued, as we will have a
...
to missingness in $Y$. Let's start by add some missingness to our outcome.
