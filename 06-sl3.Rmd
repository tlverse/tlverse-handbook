...
\\begin{enumerate}{
  \\item Select a loss function that is appropriate for the functional parameter 
    to be estimated.
  \\item Assemble an ensemble of learners based on the properties that identify 
    what features they support.
  \\item Customize learner tuning parameters to incorporate a diversity of 
    different settings.
  \\item Select a subset of available covariates and pass only those variables 
    to the modeling algorithm.
  \\item Fit an ensemble with nested cross-validation to obtain an estimate of 
    the performance of the ensemble itself.
  \\item Obtain `sl3` variable importance metrics.
  \\item Interpret the discrete and continuous Super Learner fits.
}
