...
identifiability, is required to arrive at a target *statistical* estimand. See 
[causal target parameters](#causal) for more detail on causal models and 
identifiability. 
...
("learners" in the `sl3` nomenclature) that align with the knowledge about the 
...
`r if (knitr::is_latex_output()) '\\begin{shortbox}\n\\Boxhead{Constructing a library that respects the statistical model}'`
...
2. For each fold $v$:
    ...
    c. Use the $k$ trained learners created in the previous step to map the 
       ...
       into predicted outcomes. This results in an $n_{validation} \times k$ 
 ...
 4. Retain the observed outcomes ($Y$) for all of the $n$ observations and use
    them to measure the "loss" of each cross-validated prediction
    (e.g., ($Y - \hat{Y}^2$).
 5. For each $k$ column, take the mean across all of the $n$ losses, which we 
    call the "cross-validated empirical risk". The cross-validated empirical 
    risk provides, for each of the $k$ learners, a summarized measure of 
    performance across all of the $n$ observations. Note that if there are 
    observational-level weights in the data, then these will be taken into 
    account (i) in the cross-validated empirical risk via calculation of a 
    weighted mean across all of the $n$ losses, and (ii) in training the 
    learners, but only for learners that support weights. One can specify 
    observational-level weights in `sl3` by populating `weights` argument in 
    the `sl3_Task` with the column name in the data (character) that contains
    them.
 6. Fit the so-called "metalearner" to establish the weighted combination (a 
    $k$-dimensional vector) of the $k$ learners. The metalearner 
    is a function that decides on the weights to be assigned to
    each of the $k$ learners; taking as input the cross-validated empirical risk
    for all $k$ learners (a), or taking as input a loss function and the meta-level dataset (b).
