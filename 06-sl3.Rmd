...
We can also get the CV predictions "by hand", by tapping into each of the folds, 
and then using the fitted candidate learners (which were trained to the training
...
Similar to how we got the CV predictions "by hand", we can also calculate the 
CV risk. Specifically, this is done by tapping into each of the folds, and then 
using the fitted candidate learners (which were trained to the training set for 
...
```{r cvsl-risk-summary-handbook, echo = FALSE, eval=F}
if (knitr::is_latex_output()) {
  cv_sl_fit$cv_risk[,c(1:3)] %>%
    kable(format = "latex")
} else if (knitr::is_html_output()) {
...
#### Revere-based predictive performance for SL {-}

We can also obtain the so-called "revere"-based predictive performance for the 
SL, which is a partial CV risk for the SL, where the SL candidate learner fits 
are cross-validated but the meta-learner fit is not. The revere-based risk can 
be thought of as an approximate lower bound on the true CV risk estimate for 
the SL when the meta-learner is a simple model, like NNLS. We can output the 
...
dataset (see Figure 2 in @rvp2022super for explanation of meta-level dataset). 
This is why revere-based risks are not a true CV risk. If the meta-learner is 
not a simple regression function, and instead a more data-adaptive learner
(e.g., random forest) is used as a meta-learner, then the revere-based risk of 
the resulting SL is likely a worse approximation of the actual CV risk. This is 
because more data-adaptive learners are more likely to overfit. When simple 
parametric regressions are used as a meta-learner, like what we considered 
(NNLS with `Lrnr_nnls`), and like all of the default meta-learners in `sl3`, 
then the revere-based risk is a quick way to examine an approximation of the CV 
risk estimate of the SL and it can thought of as an approximate lower bound on 
it.
