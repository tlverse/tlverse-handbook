...
in the data are necessary to adequately model it. For other data, parametric 
...
The super learner (SL) solves the issue as it can consider many algorithms 
together, from the simplest parametric regressions to the most complex machine 
learning algorithms (neural nets, support vector machines, etc.). Additionally, 
it is proven to perform as well as possible in large samples, given the learners 
considered [@vdl2007super]. The SL represents an entirely pre-specified, data-
adaptive, and theoretically grounded approach for predictive modeling. It has 
been shown to be adaptive and robust in a variety of applications, and even in 
small samples. 

Detailed descriptions outlining the SL procedure are widely available 
[@polley2010super; @naimi2018stacked]. A pre-print article concerning practical 
considerations for specifying a SL, which also provides an overview of the 
procedure, is also now available [@rvp2022super]. This chapter focuses on 
introducing `sl3`, the standard software package for SL in the `tlverse`. 
...
# TODO
```
Consider the [_WASH Benefits Example Dataset_](#wash) from [Chapter 3](#data). 
The aim of the WASH Benefits Study is to understand how WASH (water quality, 
sanitation, hand washing, and nutrition) interventions impact the health and 
development of children born in low-income countries during their first two 
years of life. In our example dataset, there are several covariates measured at 
baseline (denoted $W$) that quantify aspects of the household and mother, like 
if there is electricity and the mother's age; the treatment ($A$), which is a 
binary variable indicating if the pregnant mother was randomized to receive a 
WASH intervention ($A=1$) or if the pregnant mother was not randomized to 
receive a WASH intervention ($A=0$); and the outcome variable ($Y$), which is 
the weight-for-height Z-score of the mother's future child. Consider the average 
treatment effect (ATE) as the target estimand, denoted
$E\Big[E(Y|A=1,W)−E(Y|A=0,W)\Big]$, where the outer expectation is taken over
$W$. In this chapter, we will show how we can use the SL to estimate the 
prediction function for $E(Y|A,W)$, the conditional mean outcome, given 
treatment and baseline covariates. It is necessary to estimate $E(Y|A=1,W)$ and 
$E(Y|A=0,W)$ components of the target estimand. More generally, we will show 
how to estimate $E(Y|X)$ In particular, first we'd need 
estimate the prediction function $E(Y|A,W)$. This involves training/fitting an 
algorithm that can take as input values on $A$ and $W$ and then output a 
predicted value for $Y$. After the algorithm has been fitted/trained, we have an 
estimate of $E(Y|A,W)$, and we can use that to obtain estimates of $E(Y|A=1,W)$ 
and $E(Y|A=0,W)$. By creating two datasets, one where $A$ is 
set to 1 for everyone and another where $A$ is set to 0 for everyone, both with $W$ keptone provide it with input data that reflects the 
scenario $A=1,W$ to obtain $E(Y|A=1,W)$ and E(Y|A=0,W); specifically, 
the baseline covariate information $W$ would remain the same treatment as it is 
in the observed data, but the treatment $A$ would be set to 1 for all individuals, 
...
In [Chapter 1](#intro), we introduced [_The Roadmap_](#roadmap) as a general 
template for using data to learn answers to (possibly causal) questions. The 
first few steps of the roadmap define the statistical estimation problem. This 
involves the translation of the question of interest into a mathematical 
formulation that can be written as a function of the data, the target estimand, 
and the specification of the statistical model. Afterwards, a set of 
instructions for approximating the target estimand from data, the estimator, can 
be constructed. 

In this chapter, we introduce the super learner (SL) as an 
estimator of prediction functions. Prediction functions are those that 
take as input data on predictors/covariates and output a predicted value. 
Predictive modeling is used in a variety of data analytic scenarios, including 
risk prediction, causal inference, and forecasting. Prediction tasks are 
commonly embedded in another estimation procedure, such as targeted minimum loss-
based estimation (TMLE), where predictive modeling is necessary for estimating 
outcome regressions and propensity scores. 

There are various strategies that can be employed to model relationships 
from data (which we refer to interchangeably as "estimators", "algorithms", and 
"learners"), and there is no "one size fits all" approach in the real world. However, 
performance metrics (e.g., mean squared error, MSE) can be used to 
compare one algorithm to another. Therefore, algorithm selection should be 
driven criteria that have been (i) proven to 
optimize relevant statistical properties (i.e., provides theoretical guarantees) 
and (ii) shown to be reliable in practice (e.g., with complex real-world data). 
The SL is an algorithm that is equipped with such a standard, the cross-
validation 
...
The set of algorithms considering by the SL, which we refer to as the "library", 
should consist of learners that align with what's known about the experiment/
study that generated the data, the and what is not known about it). In other words, 
the learners in the library should be tailored to respect the statistical model 
$\mathcal{M}$ (mentioned in both in terms of 
