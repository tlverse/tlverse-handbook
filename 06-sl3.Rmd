...
2. A statistical model $\M$ as a set of possible probability distributions that 
   could have given rise to the data. It's essential for $\M$ to only 
...
3. A translation of the scientific question of interest into a function of 
   ...
   interested in the average difference in mean outcomes under treatment $A=1$ 
   versus placebo $A=0$: 
...
Occasionally, the prediction function itself is the target estimand, but more 
commonly, the prediction function is a component of a target estimand. 

For example, we might want to estimate the prediction function for 
...
criterion [@vdl2003unified; @vaart2006oracle], which is described further in 
the ["Theoretical foundations"](#sl3-theory) subsection. Also, 
...
overview](#sl3-steps) below). More flexible metalearners, 
...
0. Fit each $k=1,\ldots,K$ learner on the whole sample of $n$ observations to 
   obtain $K$ so-called "full-fit" learners. The SL is defined by these $K$ 
   full-fit learners and the weights (also commonly referred to as 
...
2. For each fold $v$:
 ...
 4. Fit the so-called "metalearner" to establish the weighted combination of 
    the $K$ learners. The metalearner is a function that outputs the 
    weights/coefficients to be assigned to the $K$ learners, taking as input 
    the $n \times K$ meta-level dataset of cross-validated predictions, and the 
    $n \times 1$ vector of observed outcomes. If the metalearner is not equipped 
    with an objective function to optimize over (such as discrete SL), then the
    metalearner will also require a loss function or risk function as input.  

     a. Discrete SL (or cross-validated selector) metalearners first calculate 
        the so-called "cross-validated empirical risk" for 
        each learner (it's derivation is described in (i) and (ii) below), then 
        ...
        outputting a $1 \times K$ vector of weights 
        $\bm{\beta}=(\beta_1,\ldots,\beta_K)$. The discrete SL's predictions 
        will be identical to one of the learner's predictions, since the 
        discrete SL allocates all of it's weight to one single learner. As a 
        simple example, say the learners are ridge, lasso, and unpenalized 
        linear regression, and the lasso had the lowest cross-validated 
        empirical risk. Here, the weighted combination would look like 
        ...
        $\hat{Y_{SL,j}}=\sum{k=1}^3\beta_k\times\hat{Y_{k,j}}$.
        
            i. If a loss function is supplied, then the cross-validated 
               empirical risk for the $K$ learners is calculated by first 
               deriving an $n \times K$ matrix of losses: for each learner $k$ 
               and subject $i=1,\ldots,n$ (i.e., for each column and each row, 
               respectively, of the meta-level dataset of cross-validated 
               predictions), the pairing of the observed outcome $Y_i$ and 
               predicted outcome $\hat{Y_{k,i}}$ is used to calculate the loss, 
               such as the squared error loss $(\hat{Y_{k,i}}-Y_i)^2$. Next, 
               the learner-specific (i.e., column-wise) mean across the $n$ 
               losses is calculated, resulting in a $1 \times K$ vector of 
               cross-validated empirical risks (such as mean squared errors).
           ii. If a risk function is supplied instead of a loss function, then 
               the $1 \times K$ vector of learner-specific cross-validated 
               empirical risks is calculated by providing the risk function with 
               the $n \times 1$ observed outcomes and applying it at the 
               learner-level (i.e., column-wise) to the $n \times K$ meta-level 
               dataset of cross-validated predictions. An example risk function 
               that may be of interest when the goal of the SL is simply 
               prediction is 1-predictiveness, where "predictiveness" could be 
               a metric like AUC (the area under the receiver operating 
               characteristic curve). 
               
     b. Continuous SL (often what traditional "Super Learner" refers to) 
        metalearners learn which weights/coefficients result in a weighted 
        combination whose cross-validated empirical risk is minimized. Unlike 
        the discrete SL, the continuous SL directly optimizes the weights. 
        ...
        subsection). Specifically, NNLS finds the weights $\bm{\beta}$ 
        optimizing $\argmin_{\bm{\beta}}\norm{\bm{\beta}\hat{\bm{Y}}-\bm{y}}$ 
        subject to $\bm{\beta} \geq 0$, where $\norm{\dot}_2$ is the 
  ...
  5. The $K$ learner full-fits obtained in step 0 and the $\bm{\beta}$ weights 
     vector obtained in step 4 define the SL. We can now obtain SL predictions 
     on new covariate data through the weighted combination of the learner's 
     full-fit predictions' on this new covariate data.
