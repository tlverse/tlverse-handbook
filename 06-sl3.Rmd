...
stack <- make_learner(
  Stack,
  lrnr_glm, lrnr_ridge, lrnr_lasso, xgb_fast
)
```
...
setting `ntree = 20`). Any learner with an "importance" method can be used in 
`Lrnr_screener_importance`, and this currently includes `Lrnr_ranger`, `Lrnr_randomForest`, `Lrnr_xgboost`. 
...
```{r screener}
lilforest <- Lrnr_randomForest$new(num.trees = 20, write.forest = FALSE, 
                                   importance = "impurity_corrected")
screen_rf <- Lrnr_screener_importance$new(lilforest, num_screen = 5)
...
importance_plot(
  washb_varimp, main = "sl3 Variable Importance for WASH Benefits Example Data"
)
```
<!-- Explain summary!!!! -->
