...
It took `r round(as.numeric(runtime_sl_fit["elapsed"]),1)` seconds, `r round(as.numeric(runtime_sl_fit["elapsed"])/60,1)` minutes, to fit the SL 
prediction function. 
...
#### SL and candidate learner predictions
...
analytic dataset, i.e., all of the data supplied as `data` to `sl3_Task`. 
...
```
```{r predobs-plot, out.width = '60%', fig.asp = .62}
...
**`predict_fold`**: The first option to obtain CV predictions, 
`cv_preds_option1`, used the `predict_fold` function to obtain validation set 
predictions across all folds. This function only exists for learner fits that 
are cross-validated, like those in `Lrnr_sl`. In addition to supplying 
`fold_number = "validation"` in `predict_fold`, we can set 
`fold_number = "full"` to obtain predictions from learners fit to the entire 
dataset (i.e., all of the data supplied to `sl3_Task`). For instance, below we 
show that `glm_preds` we calculated above can also be obtained by setting 
`fold_number = "full"`.
...
**CV predictions "by hand"**: We can get the CV predictions by hand, by tapping 
into each of the folds, and then using the fitted candidate learners (which 
were trained to the training set for each fold) to predict validation set 
outcomes (which were not seen in training).
...
**CV risk "by hand"**: Similar to how we got the CV predictions by hand, we 
can also calculate the CV risk in a way that exposes the procedure. 
Specifically, this is done by tapping into each of the folds, and then using the 
fitted candidate learners (which were trained to the training set for 
each fold) to predict validation set outcomes (which were not seen in training) 
and then measure the predictive performance (i.e., risk). Each candidate 
learner's fold-specific risk is then averaged across all folds to obtain the 
CV risk. The function `cv_risk` does all of this internally and we show how 
to do it by hand below, which can be helpful for understanding the CV risk
...
SL using `cv_sl`. Like before we called `sl$train`, we will set a random number 
...
It took `r round(as.numeric(runtime_cv_sl_fit["elapsed"]),1)` seconds, `r round(as.numeric(runtime_cv_sl_fit["elapsed"])/60,1)` minutes, to fit the SL 
prediction function. 
...
#### Revere-CV predictive performance for SL

We can also obtain the so-called "revere"-CV predictive performance for the 
SL, which is a partial CV risk for the SL, where the SL candidate learner fits 
are cross-validated but the meta-learner fit is not. It takes
essentially no extra time to calculate a revere-based risk estimate of the SL,
since we already have the cross-validated fits of the candidates. This 
isn't to say that revere-CV SL can replace actual CV SL, but that it can be 
used to quickly examine an approximate lower bound on the SL's CV risk 
*when the meta-learner is a simple model*, like NNLS. We can output the 
revere-based risk estimate by setting `get_sl_revere_risk = TRUE` when we
call `cv_risk`.
...
```{r sl-revere-risk-handbook, eval = FALSE, echo = FALSE}
if (knitr::is_latex_output()) {
  cv_risk_w_sl_revere[,c(1:3)] %>%
    kable(format = "latex")
} else if (knitr::is_html_output()) {
...
The reason why this is not a true cross-validated risk is because the 
`cv_meta_fit` object, which is the trained meta-learner, was previously fit to 
the *entire* matrix of CV predictions from *every* fold, i.e., the meta-level 
dataset (see Figure 2 in @rvp2022super for a visual overview of the meta-level 
dataset). This is why revere-based risks are not a true CV risk. If the 
meta-learner is not a simple regression function, and instead a more 
data-adaptive learner (e.g., random forest) is used as a meta-learner, then the 
revere-based risk of the resulting SL is likely a worse approximation of the 
actual CV risk. This is because more data-adaptive learners are more likely to 
overfit. When simple parametric regressions are used as a meta-learner, like 
what we considered (NNLS with `Lrnr_nnls`), and like all of the default 
meta-learners in `sl3`, then the revere-based risk is a quick way to examine an 
approximation of the CV risk estimate of the SL and it can thought of as an 
approximate lower bound on it. This idea holds in our example; that is, with 
the simple NNLS meta-learner the revere-based risk estimate of the SL 
(`r round(sl_revere_risk, 4)`) is very close to, and slightly lower than, the CV 
risk estimate for the SL 
(`r round(cv_sl_fit$cv_risk[nrow(cv_sl_fit$cv_risk),3], 4)`). 
...
section, `sl`, is an eSL since it used NNLS as the meta-learner. We rename `sl`
to `eSL_metaNNLS` below to clarify that this is an eSL that uses NNLS as its
meta-learner.
...
We included as candidates in the dSL: (i) the same eSL as before; (ii) the 
learners considered as candidates in (i); (iii) an eSL that considered the same 
candidate learners as (i) and convex combination-constrained NNLS as the 
meta-learner; (iv) an eSL that considered the same candidate learners as (i) 
and a lasso meta-learner, using as lasso meta-learner the already 
instantiated `lrn_lasso` that we created above; (v) an eSL that considered the 
same candidate learners as (i) and an EARTH meta-learner, using as EARTH 
meta-learner the already instantiated `lrn_earth` that we created above; (vi) an 
eSL that considered the same candidate learners as (i) and a ranger 
meta-learner, using as ranger meta-learner the already instantiated 
`lrn_ranger` that we created above; and (vii) an eSL that considered the same 
candidate learners as (i) and a highly adaptive lasso (HAL) meta-learner, 
using as HAL meta-learner the already instantiated `lrn_hal` that we created 
above. Even though we did not consider this in this illustrative example, 
different sets of candidate learners can be considered in the eSLs.

Running this many eSLs in the dSL is very computationally intensive, as it is 
akin to running cross-validated SL for each eSL. Parallel programming (reviewed 
below) is recommended for training learners that are computationally intensive, 
like `dSL` defined above. That is, a parallel processing scheme should be 
defined before calling `dSL$train(task)`.

### Parallel Processing 

It's straightforward to take advantage of `sl3`'s built-in parallel processing 
support, which draws on the [`future` R package](https://cran.r-project.org/web/packages/future/index.html), which
...
```{r varimp-plot, out.width = '60%', fig.asp = .62}
