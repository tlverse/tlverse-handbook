...
nonparametric structural equation model (NPSEM), based on @pearl2009causality,
...
NPSEM \@ref(eq:npsem-shift), we may consider stochastic interventions in two
...
well -- that is, as per @diaz2012population, $\P_{\delta}(g_0)(A = a \mid W) =
g_0(a - \delta(W) \mid W)$.
...
\coloneqq \E_{P_0^{\delta}}\{Y_{d(A,W)}\}$, the mean of the counterfactual
...
parameter in \@ref(eq:identification2012) matches exactly the counterfactual
...
@diaz2018stochastic, the statistical target parameter may also be denoted
$\Psi(P_0) = \E_{P_0}{\overline{Q}(d(A, W), W)}$, where $\overline{Q}(d(A, W),
W)$ is the counterfactual outcome value of a given individual under the
