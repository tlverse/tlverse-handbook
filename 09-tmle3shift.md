...
components of the NPSEM may be thought of in two equivalent ways. A
_general_ stochastic intervention replaces the equation $f_A$, which gives rise
to $A$, and defines $g(A \mid W)$, the conditional density of A, with a
candidate density $g_{A_{\delta}}(A \mid W)$. In the absence of the
intervention, we would consider any given value of $A$ -- that is, the result of
evaluating the function $f_A$ at a given value $W = w$ -- as the result of a
random draw from the conditional density $g(A \mid W)$, that is, $A_{\delta}
\sim g_{\delta}(\cdot \mid W)$.  In applying the intervention, we simply remove
the structural equation $f_A$, instead drawing the post-intervention value
$A_{\delta}$ from the distribution defined by the candidate density
$g_{A_{\delta}}(A \mid W)$. The post-intervention value $A_{\delta}$ is
stochastically modified in the sense that it has been drawn from an arbitrary
(in practice, user-defined) distribution.
...
knowledge of the natural (i.e., intervention-free) density $g(A \mid W)$. When
$g_{A_{\delta}}(A \mid W)$ is _piecewise smooth intvertible_
[@haneuse2013estimation] (see below), there is a direct correspondence between
the post-intervention density $g_{A_{\delta}}(A \mid W)$ and a function $d(A, W;
\delta)$ that map an observed pair $\{A, W\}$ to the post-intervention quantity
...
\begin{assumption}[Piecewise smooth invertibility]
...
where the counterfactual outcome $Y_{A_{\delta}} \sim
\mathcal{P}_0^{A_{\delta}}$, defining both $g_{A_{\delta}}(A \mid W)$ and $d(A,
W; \delta)$. For the remainder of this chapter, we will focus on additive MTPs
of the form
\begin{equation}
  d(a, w) =
  \begin{cases}
...
general stochastic intervention, by considering the random draw
$\P_{A_{\delta}}(g_{0, A})(A = a \mid W) = g_{0,A}(a - \delta(W) \mid W)$, as
per @diaz2018stochastic.
...
(EIF), a key quantity required for constructing efficient estimators, in the
...
\begin{equation}
  ...
  (\#eq:eif-var-shift)
\end{equation}
allowing for Wald-style confidence intervals to be computed as $\psi_n \pm z_{1
- \alpha/2} \cdot \sigma_n / \sqrt{n}$. Under certain conditions, the resampling
based on the bootstrap may also be used to compute $\sigma_n^2$
[@vdl2011targeted].
...
can formalize this procedure by defining the shift function $\delta(A, W)$ as
follows.
\begin{equation}
  \delta(a, w) =
    \begin{cases}
...
$\delta_{\text{max}}$. To use such a stochastic intervention in practice, the
[`tmle3shift`](https://github.com/tlverse/tmle3shift) package implements the
functions `shift_additive_bounded` and `shift_additive_bounded_inv`, which
define a variation of this strategy:
\begin{equation}
  \delta(a, w) =
    \begin{cases}
      \delta, & C(\delta) \leq M \\
...
in assessing the mean counterfactual outcome over a grid of shifts $\delta$
-1, 0, 1 on the scale of the treatment $A$ (note that the
numerical choice of shift is an arbitrarily chosen set of values for this
example).
...
This MSM $m_{\beta}(\delta)$ may more generally be expressed $\beta_0 =
\text{argmin}_{\beta} \sum_{\delta}(\psi_{\delta}(P_0) - m_{\beta}(\delta))^2
h(\delta)$, which is the solution to the estimating equation
...
\begin{equation}
   ...
   (\#eq:eif-msm-shift)
\end{equation}
Here, in Equation \@ref(eq:eif-msm-simple), the first term is of dimension $d
...
$\psi_{n,\delta}$. Due to this convenience, inference based on a working MSM is
...
\beta_0 \neq 0$ and corresponding confidence intervals. Note that the parameter
of interest $\beta_0$ of the MSM is asymptotically linear (and, in fact, a TML
estimator) as a consequence of its construction from individual TML estimators.
