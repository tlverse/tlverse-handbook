...
human bias enters statistical analysis efforts in the form of improper model
selection. All procedures for estimation and hypothesis testing are derived
based on a choice of a statistical model; thus, obtaining valid estimates and
statistical inference relies critically on the chosen model
...
decision? Such knowledge can, and should, be incorporated in the statistical model.
Alternatively, the data could be from an observational study, in which there is
no control over the treatment assignment mechanism. In such case available
knowledge about the data-generating process (DGP) is even more limited.  If
...
the choice of the model, and (3) the results of significance testing (i.e.,
p-values).
...
safeguarding against weak science by providing formal techniques for 
evaluating the veracity of a claim using properly collected data, 
statistics is increasingly often used "to aid and abet weak science, a role it 
can perform well when used mechanically or ritually." [@stark2018cargo].
This presents a fundamental drive behind the epidemic of false findings 
from which scientific research is suffering [@vdl2014entering].
...
focus of (i.e., "targeting of") the scientific question at hand. This encompases running counter to the current culture problem of "convenience statistics," which opens the door to biased estimation, misleading analytic results, and erroneous discoveries. Targeted Learning (TL) embraces the fundamentals that formalized the field of statistics, notably including the notions that a statistical model must
...
fields, to equip them with the necessary knowledge and skills to utilize the
methodological developments of TL. The Targeted Learning is a principled technique for answering queries with confidence, which utilizes advances in causal inference, deep statistical theory, and machine learning  --- so that each and every data 
...
critical role in allowing practitioners to access the published results
...
other and cohesively as building blocks for formulating sophisticated
...
real-world causal analyses. Some preliminaries are required prior to
