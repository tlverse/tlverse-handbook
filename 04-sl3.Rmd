...
We address feature selection and tuning in the next section, "Extensions".
...
dt_stack <- delayed_learner_train(stack, washb_task)
plot(dt_stack, color = FALSE, height = "400px", width = "100%")
```
We visualize the stack in the above plot. If you zoom in you should be able to
see the learner names that we included in our stack of base learning algorithms.
...
correlation with the outcome (`cor.test` p-value $\leq 0.1$).
...
```
We visualize the `fancy_stack` above, which lays out these machine learning
steps with and without the feature selection pipeline.
