...
We illustrate the usage of the resubstitution method with `origami` package
...
`folds_loo(n)`, similarly to the resubstitution method, we just need the total
...
With Montecarlo cross-validation, we randomly select some fraction of the data
...
divided into two sets, a training set of $n_0 = n(1-p)$ observations and a
validation set of $n_1 = np$ observations. This process is then repeated
...
multiple times, generating (at random) new training and validation partitions
each time. In contract to the montecarlo cross-validation, the total number of
...
```{r, fig.cap="Illustration of rolling origin cross-validation"}
...
```{r, fig.cap="Illustration of rolling window cross-validation"}
...
```{r, fig.cap="Illustration of rolling origin v-fold cross-validation"}
...
```{r, fig.cap="Illustration of rolling window v-fold cross-validation"}
...
cvlm_results <- cross_validate(cv_fun = cv_lm, folds = folds,
...
folds <- make_folds(washb_data)
cvrf_results <- cross_validate(cv_fun = cv_rf, folds = folds,
                               data = washb_data, reg_form = "whz ~ .")
