...
The *continuous/ensemble Super Learner*, often referred to as *Super Learner* is
a weighted average of the library of algorithms, where the weights are chosen to
minimize the cross-validated empirical risk of the library. Restricting the
...
This entire procedure can be itself cross-validated to get a consistent estimate
of the future performance of the Super Learner, and we implement this procedure
...
Say we observe a learning data set $X_i=(Y_i,W_i)$, for $i=1, \ldots, n$, where
$Y_i$ is the outcome of interest, $W_i$ is a $p$-dimensional set of covariates,
and our objective is to estimate the function $\psi_0(W) = E(Y|W)$.  This
function can be expressed as the minimizer of the expected loss: $\psi_0(W) =
\text{argmin}_{\psi} E[L(X,\psi(W))]$. Here, the loss function is represented as
$L$ (e.g., squared error loss, $L: (Y-\psi(W))^2)$).
...
we need to specify is the algorithm to use, which is passed as `method` to
...
to used to select the optimal model is `RMSE` for continuous outcomes and
...
most important covariates according to this ranking, and we decreased the ntree
to 20.
...
any single learner (see "Cross-validation" section of this [`sl3` introductory
tutorial](https://tlverse.org/sl3/articles/intro_sl3.html)).
...
`tlverse/delayed` framework. For those like myself who are not particularly keen
on understanding the intricacies of `delayed`, let's focus on the main point of
this figure: we can see there are 10 realizations of the stack which represent
the 10 cross-validation folds and there is a separate hold-out (top branch of
the figure) that will not be used to fit the Super Learner.
...
The Super Learner algorithm fits a metalearner on the validation-set predictions
...
contradictory and confusing. Nevertheless, we like it, and so do collaborators,
so we created a variable importance function in `sl3`! The `sl3` `varimp`
function returns a table with variables listed in decreasing order of importance
...
lost. If this risk difference is zero then losing that covariate had no impact,
and is thus not important by this measure. We do this across all of the
...
```{r varimp-plot, eval=FALSE, warning = FALSE, message = FALSE}
...
* We just discussed conditional mean estimation, outcome prediction and variable
  importance. In future updates of this chapter, we will delve into prediction
  of a conditional density, and the optimal individualized treatment rule.
...
* If we plug in the estimator returned by super learner into the target
  ...
  + An asymptotically linear estimator is important to have, since they converge
    to the estimand at $\frac{1}{\sqrt{n}}$ rate, and thereby permit formal
  ...
  + Plug-in estimators of the estimand are desirable because they respect both
    the local and global constraints of the statistical model (e.g., bounds)
    and have better finite-sample properties.
...
* Super Learner is fantastic for pure prediction, and for obtaining an initial
  ...
  the desirable statistical properties mentioned above.
...
Here's a potential solution to (Exercise 3)[#sl3ex3].
...
```{r, eval=FALSE, fig.cap="Animation of how a counterfactual outcome changes as the natural treatment distribution is subjected to a simple stochastic intervention", echo=FALSE, eval=TRUE, out.width='60%'}
...
`Lrnr_density_semiparametric`, a semiparametric approach that assumes a normal
distribution and estimates moments (either the mean or the mean and variance) of
that distribution using arbitrary learning algorithms. A Super Learner may be
constructed by pooling estimates from each of these approaches.
...
lrn_hse_hal <- Lrnr_density_semiparametric$new(
  mean_learner = Lrnr_hal9001$new()
)
lrn_mvd_hal <- Lrnr_density_semiparametric$new(
  mean_learner = Lrnr_hal9001$new(),
  var_learner = Lrnr_hal9001$new()
)
sl_lrn_dens <- Lrnr_sl$new(
  learners = list(lrn_haldensify, lrn_hse_hal, lrn_mvd_hal),
...
one of the semiparametric conditional density estimation procedures, as the
nonparametric conditional density approach based on the highly adaptive lasso
[@diaz2011super; @benkeser2016hal; @coyle2018hal9001; @hejazi2019haldensify] is
currently unable to accommodate large datasets.
...
lrn_cv_mvd_hal <- Lrnr_cv$new(
  learner = lrn_mvd_hal,
  full_fit = TRUE
)

# modify learner list, using existing SL for Q fit
learner_list <- list(Y = sl_lrn, A = lrn_cv_mvd_hal)
```
