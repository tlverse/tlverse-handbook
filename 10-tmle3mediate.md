...
* for $\mathbb{E}\{Y(A_{\delta}, Z_{A_{\delta}})\}$, an efficient estimator for
  the effect of a joint intervention on both the treatment and mediation
  mechanisms, as per @kennedy2019nonparametric.
...
components of the EIF are defined as follows: $D^Y_{\delta} =
\frac{g_{\delta}(a \mid w)}{e(a \mid z, w)}\{y - \overline{Q}_{Y}(z,a,w)\}$,
$D^A_{\delta} = \frac{\delta\phi(w) \{a - g(1 \mid w)\}} {\{\delta g(1 \mid w)
+ g(0 \mid w)\}^2}$, and $D^{Z,W}_{\delta} = \int \overline{Q}_{Y}(z, a, w)
g_{\delta}(a \mid w) d\kappa(a)$, and where $\phi(w) =
\mathbb{E}\{\overline{Q}_{Y}(1, Z, W)
...
estimates towards solutions of the score equations
$\frac{1}{n}\sum_{i=1}^n D^A(O_i) = 0$ and $\frac{1}{n}\sum_{i=1}^n D^Y(O_i) =
0$, respectively.  This estimator is implemented in `tmle3mediate` package. We
demonstrate the use of `tmle3mediate` to obtain $\mathbb{E}\{Y(A_{\delta}, Z)\}$
via its TML estimator in the sequel.
