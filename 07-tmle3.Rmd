...
<!--
IM: This section is nice and clear, very simple. Given that tmle3 is more powerful
then what we describe here, I would include more details (e.g., one-step, CV-TMLE), maybe a simple
"how to do" for using tmle to build your own parameter. That can be useful for a more advanced user
-->
...
\mathbb{E}[Y \mid A=0,W]]$. We interpret this parameter as the difference in mean outcome
when treatment $A=1$ and $A=0$, averaging over the distribution of
...
to as $\overline{Q}_0(A,W)$. We denote the estimate of $\overline{Q}_0(A,W)$ as $\overline{Q}_n(A,W)$.
To construct an estimate of the ATE $\psi_n$, we only need to "plug-in" the
...
We see that the Super Learner estimates the true parameter value (indicated by the
...
form of the covariate $H_n(A,W)$ differs across target parameters; in the case
...
score. We note that the estimator depends both on the initial fit (by `sl3`) of the
...
<!--
IM: Perhaps we should be a bit more careful here: "each target parameter under
a certain statistical model has a EIF etc. Therefore, the corresponding TML estimatior etc.
-->
...
specifying a TMLE in the `tlverse`. In designing `tmle3`, we
...
introduced earlier. Our target parameter is still the average treatment effect, 
which is the effect we want to estimate using the available data.
...
<!--
IM: Cite the IPCW-TMLE paper for the reader
-->
...
Here, we use a Super Learner as defined in the previous chapter. 
...
can get extra results from the summary:
...
A `tmle3_Task`, analogous to an `sl3_Task`, contains data we're
fitting the TMLE to. It also contains the NPSEM generated from the `node_list`
...
<!--
IM: I would include some basics from tmle3_Update here (or in the appendix). Most users won't want to 
look at several different places for the basics. 
-->
...
<!--
IM: Perhaps we can have an example with each of these, especially RR. If not, 
it can be an exercise with solutions
-->
