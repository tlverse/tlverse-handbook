...
real-world scientific questions and data applications to mathematical formalisms
...
efficient estimators of the statistical estimand, which have sampling
...
repeated very many times, the underlying probability distribution generating the
data, $P_0$, would be revealed.  So, the observed data on a single unit, $O$,
...
observations of the random variable $O$, so the observed data is the collection
...
just two, or countably few, parameters, as is the case with the ubiquitously
relied upon Normal distribution. When one makes such assumptions without the
appropriate background knowledge, the chosen statistical model is said to be
misspecified, and this possible _model misspecification_ introduces bias of
unquantifiable magnitude. The philosophy used to justify such parametric
assumptions is rooted in misinterpretations of the often-quoted saying of George
Box, that "All models are wrong but some are useful," which has been
irresponsibly used to encourage the data analyst to make arbitrary modeling
choices.

The result is a practice of statistical data science in which starkly different
answers to the same scientific problem emerge, owing to the application of
...
statistical model, $\M$, to the parameter space (usually a real number) $\R$ --
formally, the target parameter is the mapping $\Psi: \M \rightarrow \R$. The
...
require an extra set of assumptions to allow their _identification from the
...
data to limit the statistical model, $\M$, in a realistic manner, so that the
true data-generating distribution, $P_0$, is contained within $\M$, which is
itself a collection of candidate probability distributions respecting the
...
effects may be defined as summaries of the population of interest (such as the
population mean of a particular outcome) under differing contrasts (comparing
the treated to the untreated condition). For example, a causal effect could be
defined as the mean difference (in the rate) of a disease outcome between two
...
the same population were set to uniformly experience high pollution levels.
...
of interest, including simply assuming that the counterfactual outcomes exist in
...
tables", or through considering interventions on structural causal models (SCMs)
...
These interventions may be thought of as surgical operations that imply changes
...
on the system removed $f_W$ as a parent of $f_A$, setting $A$ deterministically
...
\begin{equation}
  ...
  (\#eq:ate)
\end{equation}
where $\E_X$ is the mean under the theoretical (unobservable) complete data $X
= (W, Y(1), Y(0))$. Note that the complete data structure $X$ is, by its very
...
based on the covariates $W$), stochastic rules (which can even account for the
