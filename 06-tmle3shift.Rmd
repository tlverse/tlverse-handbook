...
5. Construct marginal structural models to measure variable importance in terms
   of stochastic interventions, using the grid of shift interventions.
...
semiparametric-efficient estimators may be constructed. In the sequel, we focus on
a targeted maximum likelihood (TML) estimator, for which @diaz2018stochastic
give a recipe:
...
```{r fit_tmle_wrapper, message=FALSE, warning=FALSE, cache=FALSE, eval=FALSE}
...
2. What advantages, if any, are there to targeting directly the parameters of a
